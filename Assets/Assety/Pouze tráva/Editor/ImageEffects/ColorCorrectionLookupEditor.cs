        ...
        public override void OnInspectorGUI()
        {
            ...
            if (tex && (target as ColorCorrectionLookup).basedOnTempTex != AssetDatabase.GetAssetPath(tex))
            {
                ...
                else if (GUILayout.Button("Convert and Apply"))
                {
                    ...
                    bool doImport = textureImporter.isReadable == false || textureImporter.mipmapEnabled;

                    if (doImport || textureImporter.textureCompression != TextureImporterCompression.Uncompressed)
                    {
